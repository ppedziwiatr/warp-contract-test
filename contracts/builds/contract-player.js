...
async function handle(state, action) {
    ...
    if (input.function === "deposit") {
        ...
        if (claimable) {
            ...
            // Update the balances object so that the player can be transferred to the team
    ...
    if (input.function === "allow") {
        ...
        if (balances[caller] < quantity || !balances[caller] || balances[caller] == void 0 || balances[caller] == null || isNaN(balances[caller])) {
            throw new ContractError("player - Caller balance not high enough to make a balance of " + quantity + " claimable.");
        }
    ...
    if (Array.isArray(votes)) {
        const concludedVotes = votes.filter(
            (vote) =>
                (block > vote.start + vote.voteLength ||
                    state.ownership === "single" ||
                    (vote.yays / vote.totalWeight >= vote.support && (vote.yays + vote.nays) / vote.totalWeight >= vote.quorum) ||
                    vote.nays / vote.totalWeight > vote.support ||
                    vote.totalWeight === vote.yays + vote.nays) &&
                vote.status === "active"
        );
